...
export default function DashboardHeader({ userRole, userName }: DashboardHeaderProps) {
 ...
 return (
 <div className="bg-white border-b border-gray-200 shadow-sm pr-8 pl-2 py-4 md:mt-0 mt-12 transition-all duration-300">
 ...
 className="flex items-center space-x-2 bg-gray-100 hover:bg-gray-100 rounded-lg px-3 py-2 transition-all duration-200 hover:scale-105"
 >
 <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center text-primary-foreground text-sm font-semibold">
 {userName ? userName.charAt(0).toUpperCase() : (userRole ? userRole.charAt(0).toUpperCase() : 'U')}
 </div>
 <span className="hidden sm:inline text-sm font-medium text-gray-900 truncate max-w-24">
 {userName || userRole || 'User'}
 </span>
 <svg 
 className={`w-4 h-4 text-gray-900 transition-transform duration-200 ${showProfileMenu ? 'rotate-180' : ''}`}
 fill="none" 
 stroke="currentColor" 
 viewBox="0 0 24 24"
 >
 ...
 className="flex items-center w-full px-4 py-3 text-sm text-gray-900 hover:bg-gray-100 transition-colors duration-200"
 >
