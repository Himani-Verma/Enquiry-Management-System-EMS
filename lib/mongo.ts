...
export async function connectMongo() {
  ...
  if (!uri) {
    const error = new Error('MONGODB_URI environment variable is not set. Please configure it in .env.local file.');
    console.error('❌ MongoDB connection failed:', error.message);
    console.error('📝 To fix this:');
    console.error('   1. Copy .env.example to .env.local');
    console.error('   2. Add your MongoDB connection string to MONGODB_URI');
