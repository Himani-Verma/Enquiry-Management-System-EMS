...
export default function ExecutiveEnquiriesPage() {
 ...
 // Service and subservice options - will be loaded from database
 ...
 try {
 console.log('🔄 Executive - Loading enquiries for user:', user);
 const headers = { 
 ...
 Object.values(data.services).forEach((category: any) => {
 if (Array.isArray(category)) {
 category.forEach((service: any) => {
 ...
 setServiceOptions([...serviceNames, 'General Inquiry']);
 console.log('✅ Loaded', serviceNames.length, 'services from database');
 }
 }
 } catch (error) {
 ...
 <div className="text-gray-900">Loading enquiries...</div>
 </div>
 </div>
 </div>
 );
 ...
 className="px-4 py-2 text-sm font-medium text-black bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200 transition-all duration-200 hover:scale-105"
 >
 Cancel
 </button>
 <button
 type="submit"
 className="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700 transition-all duration-200 hover:scale-105"
 >
 Add Enquiry
 </button>
 </div>
 ...
 <div className="text-gray-900">{enquiry.enquiryDetails || 'No details provided'}</div>
 </div>
 </td>
 <td className="px-6 py-4 whitespace-nowrap">
 ...
 <td className="px-6 py-4 whitespace-nowrap">
 <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 transition-all duration-200 hover:scale-105">
