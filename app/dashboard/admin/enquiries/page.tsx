...
export default function AdminEnquiriesPage() {
 ...
 // Try to load from visitors first (since that's where the actual data is)
 console.log('🔍 Loading visitors as enquiries...');
 ...
 console.log('✅ Loaded', visitors.length, 'visitors as enquiries');
 ...
 // Fallback: try to load from enquiries collection
 console.log('🔍 No visitors found, trying enquiries collection...');
 ...
 console.log('✅ Loaded', list.length, 'enquiries from enquiries collection');
 setEnquiries(list);
 ...
 console.log('🔥 DELETE FUNCTION CALLED - Version 2.0 - enquiryId:', enquiryId);
 ...
 try {
 console.log('🗑️ Starting delete process for enquiry:', enquiryId);
 console.log('🔑 Using token:', token ? 'Token exists' : 'No token');
 
 // Determine which API endpoint to use based on data source
 const deleteUrl = dataSource === 'visitors' 
 ? `${API_BASE}/api/visitors/${enquiryId}`
 : `${API_BASE}/api/analytics/delete-enquiry/${enquiryId}`;
 ...
 console.log('✅ Enquiry removed from state');
 
 // Show success message to user
 alert('✅ Enquiry deleted successfully!');
 
 // Reload enquiries to get updated list
 console.log('🔄 Reloading enquiries...');
 loadEnquiries();
 } else if (response.status === 401 || response.status === 403) {
 // Authentication/Authorization error
 console.error('❌ Authentication error:', response.status);
 ...
 alert(`Failed to delete enquiry: ${errorData.message || 'Unknown error'}`);
 setError(`Failed to delete enquiry: ${errorData.message || 'Unknown error'}`);
 }
 } catch (e) {
 console.error('❌ Error deleting enquiry:', e);
 alert(`Error deleting enquiry: ${e instanceof Error ? e.message : 'Unknown error'}`);
 ...
 <p className="text-gray-900">Add and manage enquiries from multiple channels</p>
 {dataSource === 'visitors' && (
 <div className="mt-3 text-sm bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-2 rounded-lg transition-all duration-200 hover:scale-105">
 ...
 <p className="text-sm text-gray-900">Manually add new enquiries</p>
 </div>
 </div>
 <button
 ...
 <p className="text-sm text-gray-900">From all sources</p>
 </div>
 </div>
 ...
 required
 rows={4}
 className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 placeholder-gray-500 transition-all duration-200 hover:scale-105"
 ...
 className="px-6 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 hover:scale-105"
 >
 Cancel
 </button>
 <button
 ...
 <div className="text-sm text-gray-800">{enquiry.email}</div>
 {enquiry.phoneNumber && (
 <div className="text-sm text-gray-800">{enquiry.phoneNumber}</div>
 )}
 </div>
 </td>
 <td className="px-6 py-4 whitespace-nowrap">
 ...
 <div className="text-gray-800">No enquiries found</div>
 </div>
 )}
 </div>
 </div>
